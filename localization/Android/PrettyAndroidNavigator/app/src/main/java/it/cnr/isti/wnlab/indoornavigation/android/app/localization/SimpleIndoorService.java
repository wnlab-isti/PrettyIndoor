...
import it.cnr.isti.wnlab.indoornavigation.IndoorLocalizationStrategy;
...
/**
 * In the MVC view this service implements the model and some controllers.
 * <p>
 * Firstly there is the service code.
 * Then, there is the logic divided in:
 * 1) Initialization (of data structures and components)
 * 2) Start (Note that the registration of this to the strategy actually starts everything)
 * 3) Stop (The cancellation of this observer to position updates)
 */
public class SimpleIndoorService extends Service implements Observer<IndoorPosition> {
    ...
    private void initFingerprints() {
        ...
        wifiDist = new DistancesMap<>(wiFing, (chosenStrategy == Strategies.KF_STRATEGY ? kfWifiDistancesK : pfWifiDistancesK), null);
        wifi.register(wifiDist);

        // Magnetic fingerprint
        magFing = (new MagneticFingerprintMap.Builder()).build(magFile);
        magDist = new DistancesMap<>(magFing, (chosenStrategy == Strategies.KF_STRATEGY ? kfMagneticDistancesK : pfMagneticDistancesK), null);
        mh.register(magDist);
    }
    ...
    private void stopLocalization() {
        ...
        if(wifiDist != null)
            wifi.unregister(wifiDist);
        if(magDist != null)
            mh.unregister(magDist);
        // Set flag
        localizing = false;
    }
